...
int main(int argc, char *argv[]) {
    int tamanoFisico, tamanoPagina, tamano_min_proceso, tamano_max_proceso;
    ...
    if (scanf("%d", &tamanoPagina) != 1 || tamanoPagina <= 0 || tamanoPagina > tamanoFisico) {
        printf("Error: Tamano de pagina invalido\n");
        return 1;
    }
    
    printf("Ingresa el tamano minimo de proceso (MB): ");
    if (scanf("%d", &tamano_min_proceso) != 1 || tamano_min_proceso <= 0) {
        printf("Error: Tamano minimo inválido\n");
        return 1;
    }
    
    printf("Ingresa el tamano maximo de proceso (MB): ");
    if (scanf("%d", &tamano_max_proceso) != 1 || tamano_max_proceso < tamano_min_proceso) {
...
time_t ultimo_acceso_memoria = comienzo_tiempo;
    ...
    while (corriendo_simulacion) {
        ...
        if (tiempo_actual - creacion_ultimo_proceso >= 2) {
            int resultado = crear_proceso(&mm, tamano_min_proceso, tamano_max_proceso);
            if (resultado == -2) {
...
if (transcurrido >= 30 && tiempo_actual - ultimo_acceso_memoria >= 5) {
    ...
    printf("Presione Ctrl+C para detener la simulacion\n");
        ...
        // El lomite de seguridad, ello para evitar simulaciones infinitas en pruebas
        if (iteracion > 300) { // 5 minutos máximo
            printf("\n El Limite de tiempo fue alcanzado,  se finalizo la simulacion...\n");
            corriendo_simulacion = false;
        }
    }
