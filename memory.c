...
void administrador_memoria_inicio(MemoryManager *mm, int tamanoFisico, int tamanoDePagina) {
...
int crear_proceso(MemoryManager *mm, int tamano_min, int tamano_max) {
    ...
    int tamano = tamano_min + rand() % (tamano_max - tamano_min + 1);
    int numero_pagina = (tamano + mm->tamanoDePagina - 1) / mm->tamanoDePagina;
    
    // Verificamos si hay suficiente memoria virtual
    int ram_disponible = 0, swap_disponible = 0;
    for (int i = 0; i < mm->numeroDeFrames; i++) {
    if (!mm->ram_frames[i]) ram_disponible++;
    }
    for (int i = 0; i < mm->numero_slots_swap; i++) {
    if (!mm->swap_slots[i]) swap_disponible++;
    }
    ...
    printf("[NUEVO PROCESO] PID: %d | Tamano: %d MB | Paginas: %d\n", 
           proc->id, tamano, numero_pagina);
...
int buscar_victima_pagina(MemoryManager *mm) {
    ...
    for (int i = 0; i < mm->contador_paginas; i++) {
        if (mm->pages[i].en_ram && mm->pages[i].ULTIMO_ACCESO < tiempo_mas_antiguo) {
            // Verificamos que el proceso esté activo
            bool esta_activo = false;
            for (int j = 0; j < mm->contador_procesos; j++) {
                if (mm->procesos[j].activo && mm->procesos[j].id == mm->pages[i].idProceso) {
                    esta_activo = true;
                    break;
                }
            }
...
void imprimir_estadp_memoria(MemoryManager *mm) {
    ...
    for (int i = 0; i < mm->numeroDeFrames; i++) {
        if (mm->ram_frames[i]) ram_usada++;
    }
    for (int i = 0; i < mm->numero_slots_swap; i++) {
        if (mm->swap_slots[i]) swap_utilizado++;
    }
    for (int i = 0; i < mm->contador_procesos; i++) {
        if (mm->procesos[i].activo) procesoActivo++;
    }
    ...
    printf("║ RAM: %d/%d marcos (%d%%)                 \n", 
           ram_usada, mm->numeroDeFrames, 
           mm->numeroDeFrames > 0 ? (ram_usada * 100 / mm->numeroDeFrames) : 0);
    printf("║ SWAP: %d/%d slots (%d%%)                 \n", 
           swap_utilizado, mm->numero_slots_swap,
           mm->numero_slots_swap > 0 ? (swap_utilizado * 100 / mm->numero_slots_swap) : 0);
